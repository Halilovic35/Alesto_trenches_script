...
--[Modern GUI redesign by Alesto]
-- Zadrži sve funkcije, ali GUI je sada:
-- - Samo Combat i ESP tab
-- - Moderni tabovi, sekcije, slideri, switchevi (roza kad su aktivni)
-- - Nema Info taba
-- - Sve funkcije ostaju, ali GUI je čist i moderan

-- 1. NOVI CONFIG
Config.Colors.Accent = Color3.fromRGB(255, 0, 128) -- Roza za aktivne switcheve
Config.Colors.TabActive = Color3.fromRGB(40, 40, 50)
Config.Colors.TabInactive = Color3.fromRGB(25, 30, 40)
Config.Colors.TabTextActive = Color3.fromRGB(255, 0, 128)
Config.Colors.TabTextInactive = Color3.fromRGB(200, 200, 200)

-- 2. OČISTI SVE STARE SEKCIJE I TABS
for _,v in pairs(MainFrame:GetChildren()) do if v:IsA("Frame") and v ~= TitleBar then v:Destroy() end end
...
for i,tab in ipairs(Tabs) do
    ...
    btn.Position = UDim2.new(0, 16 + (i-1)*130, 0, 4)
    btn.BackgroundColor3 = (tab==ActiveTab) and Config.Colors.TabActive or Config.Colors.TabInactive
    btn.Text = tab
    btn.TextColor3 = (tab==ActiveTab) and Config.Colors.TabTextActive or Config.Colors.TabTextInactive
    btn.TextScaled = true
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        for t,b in pairs(TabButtons) do
            b.BackgroundColor3 = (t==ActiveTab) and Config.Colors.TabActive or Config.Colors.TabInactive
            b.TextColor3 = (t==ActiveTab) and Config.Colors.TabTextActive or Config.Colors.TabTextInactive
        end
        CombatSection.Visible = (ActiveTab=="Combat")
        ESPSection.Visible = (ActiveTab=="ESP")
    end)
end
...
CombatSection.Size = UDim2.new(1, -32, 1, -110)
...
local CombatCorner = Instance.new("UICorner", CombatSection)
CombatCorner.CornerRadius = UDim.new(0, 14)
CombatSection.Visible = true

-- MODERNI SWITCH (roza kad je aktivan)
local function createSwitch(parent, label, state, callback)
    ...
    sw.Size = UDim2.new(0, 60, 0, 32)
    sw.Position = UDim2.new(1, -70, 0.5, -16)
    sw.BackgroundColor3 = state and Config.Colors.Accent or Color3.fromRGB(60,60,60)
    sw.Text = ""
...
-- HITBOX SWITCH
local hitboxFrame, hitboxSwitch = createSwitch(CombatSection, "Hitbox", META_HEAD, function(val) META_HEAD = val end)
hitboxFrame.Position = UDim2.new(0,0,0,0)

-- HITBOX SLIDER
local hitboxSlider = Instance.new("TextButton", CombatSection)
hitboxSlider.Size = UDim2.new(0, 220, 0, 32)
hitboxSlider.Position = UDim2.new(0, 0, 0, 48)
hitboxSlider.BackgroundColor3 = Config.Colors.Accent
hitboxSlider.Text = tostring(META_FOV)
hitboxSlider.TextColor3 = Config.Colors.Text
hitboxSlider.TextScaled = true
hitboxSlider.Font = Enum.Font.GothamBold
local draggingFOV = false
hitboxSlider.MouseButton1Down:Connect(function() draggingFOV = true end)
...
UserInputService.InputChanged:Connect(function(input)
    if draggingFOV and input.UserInputType == Enum.UserInputType.MouseMovement then
        local rel = math.clamp((input.Position.X - hitboxSlider.AbsolutePosition.X) / hitboxSlider.AbsoluteSize.X, 0, 1)
        local value = math.floor(rel * (MAX_HITBOX_FOV-1) + 1)
        META_FOV = value
        hitboxSlider.Text = tostring(value)
    end
end)

-- HITBOX COLOR PICKER (ostavi postojeći wheel, ali modernizuj preview)
local ColorWheel = Instance.new("ImageButton", ColorSection)
ColorWheel.Size = UDim2.new(0, 70, 0, 70)
ColorWheel.Position = UDim2.new(0, 140, 0, 8)
ColorWheel.BackgroundTransparency = 1
